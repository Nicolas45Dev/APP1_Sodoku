...
def relu_backward(X, output_grad):
    return ( X >= 0).astype(float) * output_grad
...
def bce_forward(x, target):
    Y = -target * np.log(x) - (1 - target) * np.log(1 - x)
    return np.mean(Y)
...
def train(x_train, target_train, x_val=None, target_val=None, epoch_count=100, learning_rate=0.04):
    ...
    for epoch in range(epoch_count):
        ...
        dldpredict = bce_backward(y,target_train)
        dldw = sigmoid_backward(w,dldpredict)
        x_3,w_3,b_3 = fully_connected_backward(W3, b3, h, dldw)
        dldv_x = relu_backward(v,x_3)
        x_2, w_2, b_2 = fully_connected_backward(W2, b2, g, dldv_x)
        dldu_x = relu_backward(u,x_2)
        x_1,w_1,b_1 = fully_connected_backward(W1, b1, x_train, dldu_x)

        # Training: Descent gradient
        W1 = W1 - learning_rate * w_1
        W2 = W2 - learning_rate * w_2
        W3 = W3 - learning_rate * w_3

        b1 = b1 - learning_rate * b_1
        b2 = b2 - learning_rate * b_2
        b3 = b3 - learning_rate * b_3
